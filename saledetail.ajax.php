...
$result = array();
if ($_REQUEST["cmd"] == "add") {
	if ($salesid = addRec($_REQUEST, $details)) {
...
function addRec($vars, $details)
{
	...
	$sql .= "sono, txdate, customer, totalqty, totalamount, ";
	$sql .= "icreatedby, dcreatedby) values (";
	$sql .= "'".DBQuote($sono)."',";
	$sql .= "'".DBQuote($txdate)."',";
	$sql .= "'".DBQuote($customer)."',";
	...
	for ($i=0; $i<sizeof($details); $i++) {
		...
		for ($j=0; $j<sizeof($skus); $j++) {
			...
			if ($qty != 0) {
				$sql  = "insert into salesdetail (";
				$sql .= "salesid, skuid, qty, unitprice, seq, itemid, ";
				$sql .= "icreatedby, dcreatedby) values (";
				$sql .= "'".DBQuote($salesid)."',";
				$sql .= "'".DBQuote($skuid)."',";
				$sql .= "'".DBQuote($qty)."',";
				$sql .= "'".DBQuote($unitprice)."',";
				$sql .= "'".DBQuote($seq)."',";
				$sql .= "'".DBQuote($itemid)."',";
				$sql .= "'".CurrentUserId()."',";
				$sql .= "now()";
				...
				if ($thisRow = DBFetchData($sql)) {
					...
					$sql .= "salesqty = salesqty + '".DBQuote($qty)."', ";
					$sql .= "imodifiedby = '".CurrentUserId()."', ";
					$sql .= "dmodifiedby = now() ";
					$sql .= "where skuid = '".DBQuote($skuid)."' ";
					$sql .= "  and txdate = '".DBQuote($txdate)."' ";
					...
					$sql .= "txdate, itemid, skuid, openqty, salesqty, ";
