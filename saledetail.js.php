...
function AjaxSave()
{
	try {
		...
		if (result.Code == "OK") {
			var f = document.frm1;
			f.sono.value = result.SONo;

			if (confirm("Record is saved successfully. S.O. No. = " + result.SONo + ".\nPrint out ?")) {
				var id = result.salesid;
				window.open("saleview.php?id="+id+"&print=1");
			}
...
//		msg += "Please input customer.\n";
	...
	ajax.setVar("cmd", "add");
	ajax.setVar("sono", form.sono.value);
	ajax.setVar("txdate", form.txdate.value);
	ajax.setVar("customer", form.customer.value);
	
	ajax.setVar("details", SKUGrid.GetJSONString());
	
	ajax.requestFile = "saledetail.ajax.php";
	ajax.method = "post";
...
function addItem()
{
	...
	ajax.requestFile = "ajaxgetitem.php?cp=1";
	ajax.method = "post";
