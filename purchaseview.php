...
//*****************************************************************************************
	...
	$sql .= "where purchaseid = '".DBQuote($_REQUEST["id"])."' ";
	$header = DBFetchData($sql);

	$sql  = "select a.*, b.size, b.color, c.itemno, c.itemname ";
	$sql .= "from purchasedetail a, sku b, item c ";
	$sql .= "where a.purchaseid = '".DBQuote($_REQUEST["id"])."' ";
...
<tr>
	<td nowrap><div class="functionname">Purchase Detail</div></td>
...
<table border="0" cellpadding="2" cellspacing="0" class="detailtable" width="100%">
<tr>
	<td>P.O. No.: <?=$header["pono"]?></td>
	<td>Date : <?=formatDate($header["txdate"])?></td>
</tr>
<tr>
	<td colspan="2">Supplier : <?=$header["supplier"]?></td>
</tr>
<tr>
	<td>Total Amount : <?=$header["totalamount"]?></td>
...
<tr>
	...
	<td>Unit Price : <?=$detail["unitcost"]?></td>
	<td>Qty : <?=$detail["qty"]?></td>
	<td>Amount : <?=$detail["amount"]?></td>
</tr>
</table>
...
function doPrint()
{
	window.open("purchaseview.php?id=<?=$_REQUEST["id"]?>&print=1");
}
<? } else {?>
window.print();
<? } ?>
