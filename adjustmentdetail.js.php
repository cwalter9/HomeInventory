...
function AjaxSave()
{
	try {
		...
		if (result.Code == "OK") {
			var f = document.frm1;
			f.stockadjno.value = result.StockAdjNo;

			if (confirm("Record is saved successfully. Stock Adj. No. = " + result.StockAdjNo + ".\nPrint out ?")) {
				var id = result.stockadjid;
				window.open("adjustmentview.php?id="+id+"&print=1");
			}
...
function doSave()
{
	...
	if (form.adjtype.selectedIndex < 0) {
	...
	ajax.setVar("cmd", "add");
	ajax.setVar("stockadjno", form.stockadjno.value);
	ajax.setVar("txdate", form.txdate.value);
	ajax.setVar("remark", form.remark.value);
	ajax.setVar("adjtype", form.adjtype.value);
	
	ajax.setVar("details", SKUGrid.GetJSONString());
	
	ajax.requestFile = "adjustmentdetail.ajax.php";
	ajax.method = "post";
...
function doCancel()
{
	document.location.href="adjustmentlist.php";
}
