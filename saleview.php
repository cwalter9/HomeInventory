...
//*****************************************************************************************
	...
	$sql .= "where salesid = '".DBQuote($_REQUEST["id"])."' ";
	$header = DBFetchData($sql);

	$sql  = "select a.*, b.size, b.color, c.itemno, c.itemname ";
	$sql .= "from salesdetail a, sku b, item c ";
	$sql .= "where a.salesid = '".DBQuote($_REQUEST["id"])."' ";
	...
	for ($i=0; $i<sizeof($tmp); $i++) {
		...
		if (!isset($details[$seq])) {
			...
			$details[$seq]["unitprice"] = $row["unitprice"];
			$details[$seq]["qty"] = 0;
		...
		$details[$seq]["amount"] += $row["qty"] * $row["unitprice"];
	}
...
<table border="0" cellpadding="2" cellspacing="0" class="detailtable" width="100%">
<tr>
	<td>S.O. No.: <?=$header["sono"]?></td>
	<td>Date : <?=formatDate($header["txdate"])?></td>
</tr>
<tr>
	<td colspan="2">Customer : <?=$header["customer"]?></td>
</tr>
<tr>
...
<tr>
	...
	<td>Unit Price : <?=$detail["unitprice"]?></td>
...
function doPrint()
{
	window.open("saleview.php?id=<?=$_REQUEST["id"]?>&print=1");
}
<? } else {?>
window.print();
<? } ?>
