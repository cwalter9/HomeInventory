...
//*****************************************************************************************
	...
	for ($i=0; $i<sizeof($tmp); $i++) {
		if ($lastItemId == $tmp[$i]["itemid"]) {
			$tmp[$i]["openqty"] = $closing;
		}
		if ($tmp[$i]["purchaseqty"]!=0 || $tmp[$i]["salesqty"]!=0 || $tmp[$i]["adjqty"]!=0 ||
		    ($_REQUEST["inactive"] && $tmp[$i+1]["itemid"] != $tmp[$i]["itemid"])) {
			$data[] = $tmp[$i];
		}
		$closing = $tmp[$i]["openqty"] + $tmp[$i]["purchaseqty"] - $tmp[$i]["salesqty"] + $tmp[$i]["adjqty"];
		$lastItemId = $tmp[$i]["itemid"];
	}
...
<p class="reportsubtitle">Details</p>
...
<col width='100'/><col width='100'/><col width='200'/><col width='80'/>
...
<? 
	...
	for ($i=0; $i<sizeof($data); $i++) {
		...
		if ($lastItemId != $data[$i]["itemid"]) {
			$openstyle = "class='openclose'";
			$totalopenqty += $data[$i]["openqty"];
		} else {
			$openstyle = "";
		}
		if ($i+1==sizeof($data) || $data[$i]["itemid"] != $data[$i+1]["itemid"]) {
			$closestyle = "class='openclose'";
...
?>
	<tr>
		...
		<td nowrap><?=formatDate($data[$i]["txdate"])?>&nbsp;</td>
...
<tfoot>
	<tr>
		<td colspan="4" align="right">Total <?=($_REQUEST["inactive"]?"":"(Excluded inactive items)")?></td>
